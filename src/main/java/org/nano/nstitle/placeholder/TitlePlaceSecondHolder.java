...
public class TitlePlaceSecondHolder extends PlaceholderExpansion {
    ...
    public String onPlaceholderRequest(Player p, @NotNull String params){
        if ( p == null ) return "";
        if ( params.equals("title-2")) return getAuthor();
        TitleInventory inv = registry.gettInv(p.getUniqueId());
        String name = "";
        if ( inv.getEquip(2) != null ) {
            Title title = manager.getTitle(inv.getEquip(2));
